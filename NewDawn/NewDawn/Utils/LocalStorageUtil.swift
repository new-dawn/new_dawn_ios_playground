...
class LoginUserUtil {
    ...
    static func fetchLoginUserProfile(getLocal: Bool = true, callback: @escaping (UserProfile?, String?) -> Void) -> Void {
        ...
        if let user_id = getLoginUserId(), let accessToken = getAccessToken() {
            ...
            if let user_profile: UserProfile? = LocalStorageUtil.localReadKeyValueStruct(key: LoginUserUtil.LOGIN_USER_PROFILE), getLocal == true {
